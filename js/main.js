(function(){
	...
	Util.prototype = {
		...
		getElement:function(node){
           node = typeof node == 'string'?document.getElementById('node') :node;//获取已知id的元素或给定标签名的元素
           return node;
		},
		getElementByClassName:function(str,root,tag){
            if(root){
           	    root = typeof root =='string'?document.getElementById(root):root;//根据指定的class获取元素
            }else{
                root = document.body;
            }
            tag = tag||'*';
		...
		setStyle:function(obj,attr,value){
            if(arguments.length == 2){
           	    return this.getStyle(obj,attr);//设置元素的属性
            }else{
        ...
        hasClass:function(obj, cls){  //判断元素是否具有给定的属性
            return obj.className.match(new RegExp('(\\s|^)' + cls + '(\\s|$)'));  
        },  
        addClass:function(obj, cls) {  //为给定的元素添加class
        ...
        startMove:function (obj,json,fn){
            ...
            obj.timer = setInterval(function(){ 
                for(var attr in json){
                    var flag = true;                    
                    if(attr == 'opacity'){
                        icur = Math.round(parseFloat(_this.getStyle(obj,attr))*100);//当前值获取
                    }else{
                        icur = parseInt(_this.getStyle(obj,attr));
                    }
                        speed = (json[attr]-icur)/5;//设置速度
